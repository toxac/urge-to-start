...
- challenges: //populate this using ChallengeSchema challenges related to starting/new beginning
- tasks: refer the tasks below
- notes: // populate using NoteSchema from newTYpes.ts for this quest
...
#### MotivationForm Details (based on ProfileMotivationSchema for profiles.motivations)

- field: push (checkbox group), datatype: string[]
  - label: "What are you running from?"
  - description/hint: "Select two strongest forces pushing you away from your current reality."
  - max_selecttions: 2
  - options:
...
- field: push_other (input), datatype: string
  - label: "Please specify"
  - placeholder: "What else is pushing you?"
  - note: only available if push[0] == "other"
- field: pull (checkbox group), datatype: string[]
